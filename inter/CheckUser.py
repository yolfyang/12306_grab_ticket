...
class CheckUser:
    """yolf：
    单独用线程随时监控是否登陆状态，如果未登陆自动登陆，保持随时登陆状态
    """
    def __init__(self, session):
        self.session = session

    def send_check_user(self):
        """
        检查用户登录, 检查间隔为2分钟
        :return:
        """
        ...
        while 1:
            ...
            configCommon.check_sleep_time(self.session)  # 修复晚上查询线程休眠时，检查登录线程为休眠，造成快豆迅速消耗
            if wrapcache.get("user_time") is None:
                check_user_url = self.session.urls["check_user_url"]
                data = {"_json_att": ""}
                check_user = self.session.httpClient.send(check_user_url, data)
