...
class confirmSingleForQueue:
    ...
    def data_par(self):
        ...
        if not self.passengerTicketStrList and not self.oldPassengerStr:
            ...
            getPassengerDTOsResult = s.get_passenger_ticket_strlist_and_oldpassenger_str()
    ...
    def sendConfirmSingleForQueue(self):
        ...
        try:
            if self.is_node_code:
                ...
                for i in range(3):
                    randCode = get_rand_code(is_auto_code=True, auto_code_type=2)
            ...
            checkQueueOrderResult = self.session.httpClient.send(checkQueueOrderUrl, data)
