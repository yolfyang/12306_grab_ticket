...
class getQueueCount:
    ...
    def sendGetQueueCount(self):
        ...
        :return:
        """
        getQueueCountResult = self.session.httpClient.send(self.session.urls["getQueueCountUrl"], self.data_par())
...
class queryQueueByAfterNate:
    ...
    def sendQueryQueueByAfterNate(self):
        for i in range(10):
            queryQueueByAfterNateRsp = self.session.httpClient.send(urls.get("queryQueue"))
            if not queryQueueByAfterNateRsp.get("status"):
                ...
                send_server_chan(Ticket.WAIT_ORDER_SUCCESS)
